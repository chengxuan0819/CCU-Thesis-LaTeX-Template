...
在前述的 MATLAB 邏輯沙盒中，本研究已確立動態李雅普諾夫演算法之核心參數。本節正式將該演算法與參數移植至由工研院開發、符合 3GPP 規範之 C++ 5G WiSE 系統級模擬平台（System-Level Simulator, SLS）中 \cite{jao_wise_5g}。相較於理想之單一細胞沙盒，此 5G WiSE SLS 平台導入了極具挑戰性的現網物理特性，包含動態多細胞同頻干擾（Multi-cell Interference）、隨機使用者分佈，以及真實的通道快衰落模型。
...
網路拓樸 (Topology)         & 7 Sites, 21 Cells (Wrap-around 網格模型)             \\
...
\paragraph{天線高頻切換之乒乓效應與佇列延遲發散之隨機因果關係剖析 (Causal Analysis of Ping-Pong Effect and Queue Delay Divergence)}
傳統基於負載門檻之啟發式演算法（Load-based SDA）因其決策的時間滯後性（Time Lag），在臨界負載下極易隨著流量的隨機起伏，引發毫秒級（TTI 級別）的天線頻繁啟閉（即乒乓效應）\cite{neely2010_stochastic}。此種高頻率的能耗狀態切換，在隨機排隊論與動態控制理論中，會導致 MAC/RLC 層佇列延遲發生非線性累積與災難性的發散（Queue Delay Divergence），其內在通訊物理與隨機控制因果可剖析如下：

\begin{enumerate}
    \item \textbf{服務速率方差之二次方放大（Amplification of Service Rate Variance）：}
    在隨機排隊系統中，系統的平均排隊延遲（Average Queueing Delay）除了與平均服務率相關外，更受到服務率波動程度之強烈制約。根據隨機排隊論經典之 Pollaczek-Khinchine (P-K) 公式與利特爾法則（Little's Law），平均排隊佇列長度與服務速率之統計方差（Variance, $\sigma_{\mu}^2$）呈正比關係。當基地台在低容量省電模式（16T，$ \mu_{\text{low}} $）與高容量全開模式（64T，$ \mu_{\text{high}} $）之間發生高頻率之乒乓震盪切換時，即時服務速率 $\mu(t)$ 呈劇烈之非平穩起伏，使其統計方差 $\sigma_{\mu}^2$ 呈二次方級數激增。此方差之暴增破壞了底層比例公平（PF）排程器在時域調度上的連續性，進而急劇拉高平均排隊延遲，導致系統性能退化。
    
    \item \textbf{非負邊界約束下之非對稱非線性累積（Non-linear Asymmetric Accumulation under Non-negative Bound）：}
    自系統控制維度檢視，MAC 層佇列之動態演進遵循非線性差分方程式：
    \begin{equation}
        Q(t+1) = \max \big[ Q(t) - \mu(t), 0 \big] + \lambda(t)
    \end{equation}
    式中 $\max$ 算子引入了「佇列長度恆不為負」之實體物理約束。當一波隨機突發流量 $\lambda(t)$ 湧入而天線因決策時滯卡在 16T 省電模式時，即時服務率低於封包抵達率（$\mu_{\text{low}} < \lambda(t)$），佇列瞬間嚴重積壓並產生極高的長尾延遲；然而，當突發流量結束（$\lambda(t) = 0$）而天線後知後覺地盲目開啟至 64T 時，由於此時佇列已被排空（$Q(t) \to 0$），超額提供之高服務容量 $\mu_{\text{high}}$ 被 $\max$ 算子強制過濾歸零，在實體物理上\textbf{無法}用以補償或抵消先前 16T 塞車期所累積之延遲。此種「低速期之嚴重時延無法被空閒全開期之零排隊所平均」之非對稱非線性特徵，會隨著乒乓效應之高頻開關而不斷累積，最終導致 RLC/MAC 緩衝區佇列長度呈長尾式阻塞與災難性發散。
    
    \item \textbf{流量感知大腦與非線性緩衝之控制物理優勢：}
    相對地，本研究所提之動態自適應機制，將功耗懲罰直接與 MAC 層\textbf{即時細胞累計佇列 $Q_{\text{total}}(t)$} 跨層級聯立求解。當突發封包湧入、佇列稍有積壓之瞬時，佇列加權服務增益項 $-Q_n(t) \cdot R_n(a(t))$ 立刻主導決策，大腦得以「零時滯」察覺並迅速擴展天線至 64T 窄波束進行清空；而於排空後，功耗懲罰項迅速接管大腦，系統平滑收攏回 16T 微休眠（Micro Sleep）狀態。本研究引入之非線性調控因子 $\gamma = 2.0$ 為 V(t) 自適應映射提供了對抗微幅負載波動之物理緩衝，自底層控制機制上徹底消除了乒乓效應，在保障強穩定性（Strong Stability）的前提下，徹底杜絕了延遲發散風險。
\end{enumerate}
...
\begin{figure*}[htbp] % 加星號 (*) 代表要跨越雙欄頁面
    ...
    \subfloat[Low Load 情境 ($N_{\text{UE/cell}} = 3$)]{
        \includegraphics[width=0.7\textwidth]{figures/SLS_Latency_CDF_Low.png}
        \label{fig:cdf_latency_low} 
    }
    \hfill 
    \subfloat[Light Load 情境 ($N_{\text{UE/cell}} = 6$)]{
        \includegraphics[width=0.7\textwidth]{figures/SLS_Latency_CDF_Light.png}
...
\begin{figure*}[htbp] % 加星號 (*) 代表要跨越雙欄頁面
    \centering
    \subfloat[Low Load 情境 ($N_{\text{UE/cell}} = 3$)]{
        \includegraphics[width=0.7\textwidth]{figures/SLS_UPT_CDF_Low.png}
        \label{fig:cdf_upt_low} 
    }
    \hfill 
    \subfloat[Light Load 情境 ($N_{\text{UE/cell}} = 6$)]{
        \includegraphics[width=0.7\textwidth]{figures/SLS_UPT_CDF_Light.png}
