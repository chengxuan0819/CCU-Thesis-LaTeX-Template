...
本附錄詳細列出本研究所採用之系統級模擬（SLS）設定參數。為了確保模擬環境之產業通用性、避免標準術語在翻譯上的歧義，並與 3GPP TR 38.864 網路節能（NES）技術報告中之 Annex B Set 1（Macro Scenario）基準規範達到完全一致的嚴謹對齊，本參數表之技術指標與配置內容皆以國際通訊標準英文術語進行完整表列。因技術參數項目繁多，特將表格區分為兩部分呈現：表 \ref{tab:appendix_3gpp_set1_part1} 主要涵蓋網路佈署拓撲與基站實體層配置 (Part I)，表 \ref{tab:appendix_3gpp_set1_part2} 則涵蓋終端參數、資源調度與隨機流量模型 (Part II)。
...
Antenna configurations & 
  Antenna array type: 64T64R AAS, cross-polarized \newline
  For 64T: $(M, N, P, M_g, N_g, M_p, N_p) = (8, 8, 2, 1, 1, 4, 8)$ \newline
  Antenna element spacing: $(d_H, d_V) = (0.5, 0.8)\lambda$ \newline
  Tx port configuration $(M_t, N_t) = (4, 8)$ \\ \hline
...
UE receiver configuration & 2T4R cross-polarized \\ \hline
UE antenna configuration & 
  For 4R: $(M, N, P, M_g, N_g; M_p, N_p) = (1, 2, 2, 1, 1; 1, 2)$ \newline
  Antenna element spacing: $(d_H, d_V) = (0.5, N/A)\lambda$ \\ \hline
...
Codebook feedback mode & 3GPP Type I Single-Panel / Type II codebook \\ \hline
Rank adaptation scheme & Fully dynamic rank adaptation (\texttt{uFixDLRank = 0}) \\ \hline
CSI feedback periodicity & $T_{\text{CSI}} = 20$ ms \\ \hline
CSI feedback latency & 3 slots \\ \hline
UL Channel sounding & Non-precoded SRS (4 Tx ports) \\ \hline
...
Traffic model & Bursty non-full buffer traffic (FTP Model 1 / Model 3) \\ \hline
Packet payload size & Log-normal distribution / fixed interval \\ \hline
Packet arrival process & Poisson arrival process \\ \hline
HARQ scheme & Asynchronous incremental redundancy (max 3 retransmissions) \\ \hline
...
BS hardware switching latency & State transition latency ($T_{\text{transition}}$) \\ \hline
\end{tabularx}
\end{table}